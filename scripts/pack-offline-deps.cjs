...
/**
 * 打包 odc-client 的所有 pnpm 依赖为离线 store
 *
 * 原理：
...
function cleanInstall() {
  ...
  execSync('pnpm install --frozen-lockfile --no-optional=false', {
    cwd: ROOT_DIR,
    stdio: 'inherit',
    timeout: 600000,
  });
...
async function main() {
  ...
  try {
    log('=== odc-client 离线依赖打包 (独立 store) ===', 'blue');
    log('', 'reset');
    ...
    log('', 'reset');
    log('使用方法:', 'blue');
    log('  1. 将整个 offline-deps/ 目录复制到目标机器的 odc-client/offline-deps/', 'reset');
